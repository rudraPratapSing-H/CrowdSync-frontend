...
const IndoorMap = () => {
    ...
    const fetchAll = async () => {
      ...
      try {
        ...
        const [layoutRes, countsRes, safeRes] = await Promise.all([
          ...
          fetch(`http://localhost:5000/api/recentZoneCounts?eventName=${eventName}&date=${dateStr}`),
          fetch(`http://localhost:5000/api/getSafeLimit?eventName=${eventName}&date=${dateStr}`)
        ]);
  ...
  return (
    <div>
      <button
        style={{ marginBottom: 10, padding: '6px 16px', fontWeight: 600, borderRadius: 6, background: showHeatmap ? '#1976d2' : '#eee', color: showHeatmap ? '#fff' : '#222', border: '1px solid #1976d2', cursor: 'pointer' }}
      ...
      {sensors.map((sensor, idx) => {
        ...
        console.log('Zone:', zoneName, 'Count:', count, 'Safe Limit:', safeLimit);
        ...
        return (
          <React.Fragment key={`sensor-${idx}`}> 
            ...
            >
              <Tooltip>{zoneName} (Safe: {safeLimit})</Tooltip>
            </Polygon>
            <CircleMarker
            ...
            >
              <Tooltip direction="top" offset={[0, -10]} opacity={1} permanent>
                <span style={{ background: '#222', color: '#fff', padding: '2px 6px', borderRadius: 4, fontWeight: 600, fontSize: 16 }}>
                  {count}
                </span>
              </Tooltip>
            </CircleMarker>
